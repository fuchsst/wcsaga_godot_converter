...
class TableDataConverter:
    ...
    def convert_table_file(self, table_file: Path) -> bool:
        ...
        try:
            ...
            converter = ConverterFactory.get_converter_for_file(
                table_file, self.source_dir, self.target_dir
            )
    ...
    def _aggregate_stats(self, converter) -> None:
        ...
        if "relationship_mappings" in registries:
            self.conversion_stats["relationships_mapped"] += len(
                registries["relationship_mappings"]
            )
            # Update specific registries
            for mapping_type, mapping_data in registries[
