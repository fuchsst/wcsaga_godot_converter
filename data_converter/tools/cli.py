...
def main():
    ...
    parser.add_argument(
        "--source", type=Path, required=True, help="Path to WCS source directory"
    )
    parser.add_argument(
        "--target", type=Path, required=True, help="Path to Godot project directory"
    )
    parser.add_argument("--file", type=Path, help="Convert specific table file")
    parser.add_argument(
        "--validate", action="store_true", help="Validate converted resources"
    )
    parser.add_argument(
    ...
    try:
        ...
        if args.file:
            ...
            table_files = list(args.source.glob("**/*.tbl")) + list(
                args.source.glob("**/*.tbm")
            )
            ...
            print(
                f"Converted {success_count}/{len(table_files)} table files successfully"
            )
