...
class AssetMapper:
    ...
    def generate_mapping(self) -> Dict[str, Any]:
        ...
        for i, (entity_name, table_type) in enumerate(table_entities.items()):
            logger.info(
                f"  ({i+1}/{len(table_entities)}) Processing entity: {entity_name}"
            )
        ...
        logger.info(
            f"Generated mapping for {len(self.asset_mappings)} entities with {project_mapping['metadata']['total_assets']} total assets."
        )
    ...
    def _extract_entities_from_tables(
    ...
    def _create_mapping_for_entity(self, entity_name: str, table_type: TableType):
        ...
        logger.debug(
            f"Creating mapping for entity: '{entity_name}' from table: {table_type.value}"
        )
        ...
        relationships = self.asset_discovery.discover_entity_assets(
        ...
        for rel in relationships:
            ...
            if file_hash and file_hash in self.file_hash_cache:
                ...
                logger.debug(
                    f"Duplicate found for {rel.source_path}, mapping to {rel.target_path}"
                )
            else:
        ...
        primary_asset = next(
            (
                ...
                if r.relationship_type == "primary_model" or r.asset_type == "model"
            ),
            None,
        )
    ...
    def _find_primary_model(
        self, entity_name: str, table_type: TableType
    ) -> Optional[str]:
    ...
    def _scan_and_map_unmapped_files(self):
        ...
        logger.info(
            f"Found {len(all_source_files)} total files. Checking for unmapped assets..."
        )
        unmapped_count = 0
    ...
    def _build_final_json(self) -> Dict[str, Any]:
        ...
        total_assets = sum(
            len(m.related_assets) + (1 if m.primary_asset else 0)
            for m in self.asset_mappings.values()
        )
        ...
        project_mapping = {
            ...
            "statistics": {
                "ships": len(
                    [m for m in self.asset_mappings.values() if m.entity_type == "ship"]
                ),
                "weapons": len(
                    [
                        m
                        for m in self.asset_mappings.values()
                        if m.entity_type == "weapon"
                    ]
                ),
                "asteroids": len(
                    [
        ...
        for entity_name, mapping in self.asset_mappings.items():
            project_mapping["entity_mappings"][entity_name] = {
                ...
                "related_assets": [
                    self._serialize_relationship(rel) for rel in mapping.related_assets
                ],
                "metadata": mapping.metadata,
            }

            all_assets = (
                [mapping.primary_asset] if mapping.primary_asset else []
            ) + mapping.related_assets
    ...
    def _serialize_relationship(
        self, relationship: Optional[AssetRelationship]
    ...
    def save_mapping_json(
        self, project_mapping: Dict[str, Any], output_path: Path
    ) -> bool:
...
def main():
    ...
    parser.add_argument(
        "--source", type=Path, required=True, help="WCS source directory"
    )
    parser.add_argument(
        "--output",
        type=Path,
        required=True,
        help="Output JSON file for the asset mapping",
    )
    parser.add_argument(
        "--target-structure",
        type=Path,
        help="Path to a JSON file describing the target directory structure",
    )
    parser.add_argument(
