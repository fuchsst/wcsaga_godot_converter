    ...
    def read_archive(self) -> bool:
        ...
        try:
            with open(self.vp_file_path, 'rb') as f:
                ...
                sig = f.read(4)
                if sig != b'VPVP':
                    logger.error(f"Invalid VP file signature for {self.vp_file_path}. Expected VPVP, got {sig}")
                    return False
                ...
                for i in range(self.header.num_files):
                    try:
                        ...
                        if len(offset_size_data) < 8:
                            logger.warning(f"Failed to read complete offset/size data for entry {i}, possibly end of file")
                            break
                        ...
                        if len(name_bytes) < 32:
                            logger.warning(f"Failed to read complete name data for entry {i}, possibly end of file")
                            break
                        
                        name = name_bytes.decode('utf-8', errors='ignore').split('\x00')[0]  # Null-terminated string
                        
                        # Read the 4-byte timestamp
                        timestamp_data = f.read(4)
                        if len(timestamp_data) < 4:
                            logger.warning(f"Failed to read complete timestamp data for entry {i}, possibly end of file")
                            break
                        ...
                        logger.error(f"Struct error reading entry {i}: {e}. Skipping entry.")
                        ...
                        logger.error(f"Unexpected error reading entry {i}: {e}. Skipping entry.")
                ...
                logger.info(f"Successfully read index of {len(self.file_entries)} files from {self.vp_file_path.name}")
                return True
            ...
            logger.error(f"Failed to read VP archive {self.vp_file_path}: {e}", exc_info=True)
            return False
        ...
        try:
            with open(self.vp_file_path, 'rb') as f:
                for i, entry in enumerate(self.file_entries):
                    logger.debug(f"Extracting ({i+1}/{len(self.file_entries)}): {entry.name}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='List and extract files from Wing Commander Saga .vp archives.')
    parser.add_argument('vp_file', type=Path, help='Path to the .vp archive file.')
    parser.add_argument('-l', '--list', action='store_true', help='List the contents of the archive.')
    parser.add_argument('-x', '--extract', type=Path, metavar='OUTPUT_DIR',
