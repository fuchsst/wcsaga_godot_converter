...
class AfterburnerStats:
    ...
    acceleration_mult: float = (
        1.0  # Multiplier for normal acceleration when afterburning
    )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ShipPhysics":
        ...
        if "max_velocity" in data:
            if isinstance(data["max_velocity"], dict):
                ...
                physics.max_velocity = VelocityVector(
                    forward=val, reverse=val, side=val
                )

        if "afterburner_velocity" in data:
            # Convert to VelocityVector format
            val = (
                float(data["afterburner_velocity"])
                if data["afterburner_velocity"]
                else 0.0
            )
            physics.afterburner_velocity = VelocityVector(
                forward=val, reverse=val, side=val
            )
