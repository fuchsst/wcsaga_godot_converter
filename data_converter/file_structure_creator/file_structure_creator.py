...
"""
            ...
            for entity_name, asset_mapping in asset_mappings.items():
                try:
                    ...
                    if entity_type == "ship":
                        ...
                        entity_dir = (
                            self.output_root / "entities" / "fighters" / 
                            faction / category / self._sanitize_name(entity_name)
                        )
                        
                    elif entity_type == "weapon":
                        entity_dir = (
                            self.output_root / "entities" / "weapons" / 
                            self._sanitize_name(entity_name)
                        )
                        
                    elif entity_type == "effect":
                        entity_dir = (
                            self.output_root / "entities" / "effects" / 
                            self._sanitize_name(entity_name)
                        )
                        ...
                        entity_dir = (
                            self.output_root / "entities" / "environment" / 
                            self._sanitize_name(entity_name)
                        )
                    ...
                    data_dir = (
                        self.output_root / "data" / f"{entity_type}s" / 
                        self._sanitize_name(entity_name)
                    )
                    ...
                    logger.warning(f"Failed to create directories for {entity_name}: {e}")
                    continue
    ...
    def _get_asset_faction(self, asset_name: str) -> str:
        ...
        if any(pattern in name_lower for pattern in [
            ...
            "confed", "terran", "tc_", "hermes", "rapier", "arrow"
        ]):
            return "confed"
