...
class POFVersionHandler:
    ...
    def apply_version_specific_fixes(
        self, parsed_data: Dict[str, Any], version: int
    ) -> Dict[str, Any]:
        """
        ...
        if not config.get("has_mass_properties", True):
            ...
            if "header" in fixed_data and "mass" not in fixed_data["header"]:
                ...
                fixed_data["header"]["moment_inertia"] = [
                    [1, 0, 0],
                    [0, 1, 0],
                    [0, 0, 1],
                ]
    ...
    def generate_compatibility_report(
        self, version: int, chunks_found: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        if not validation["valid"]:
            ...
            report["recommendations"].append(
                "Use with caution - format may be incompatible"
            )
            return report
        ...
        missing_chunks = [
            chunk for chunk in required_chunks if chunk not in chunks_found
        ]
        ...
        unsupported_chunks = [
            chunk for chunk in chunks_found if chunk not in all_supported_chunks
        ]

        if unsupported_chunks:
            report["warnings"].append(
                f"Unsupported chunks for this version: {unsupported_chunks}"
            )
            report["recommendations"].append(
                "These chunks will be ignored during processing"
            )
...
def generate_compatibility_report(
    version: int, chunks_found: List[str]
) -> Dict[str, Any]:
