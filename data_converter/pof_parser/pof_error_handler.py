...
class POFErrorRegistry:
    ...
    def register_error(self, error: POFError) -> bool:
        ...
        if error in self._errors:
            ...
            self._duplicate_count[error_key] = (
                self._duplicate_count.get(error_key, 1) + 1
            )
            return False
        else:
...
class UnifiedPOFErrorHandler:
    ...
    def set_chunk_context(
        self, chunk_id: Optional[int], chunk_name: Optional[str] = None
    ) -> None:
    ...
    def _log_error(self, error: POFError, is_new: bool) -> None:
        ...
        if not is_new:
            ...
            if duplicate_count > 1:
                logger.debug(
                    f"Suppressed {duplicate_count-1} duplicate errors like: {error.message}"
                )
    ...
    def _track_error_type(self, error: POFError) -> None:
        ...
        self._error_count_by_type[category_key] = (
            self._error_count_by_type.get(category_key, 0) + 1
        )
    ...
    def add_io_error(self, message: str, **kwargs) -> POFError:
        """Add I/O error."""
        return self.add_error(
            message, severity=ErrorSeverity.ERROR, category=ErrorCategory.IO, **kwargs
        )
    ...
    def get_error_statistics(self) -> Dict[str, Any]:
        ...
        statistics = {
            ...
            "duplicate_errors_suppressed": sum(
                count - 1
                for count in self.registry._duplicate_count.values()
                if count > 1
            ),
            "session_duration": 0,
    ...
    def create_recovery_plan(self) -> Dict[str, Any]:
        ...
        all_errors = self.registry.get_all_errors()
        critical_errors = [
            e for e in all_errors if e.severity == ErrorSeverity.CRITICAL
        ]
        ...
        validation_errors = [
            e for e in errors if e.category == ErrorCategory.VALIDATION
        ]
        compatibility_issues = [
            e for e in warnings if e.category == ErrorCategory.COMPATIBILITY
        ]
    ...
    def format_error_report(
        self, include_debug: bool = False, max_errors: int = 50
    ) -> str:
        ...
        lines.append(
            f"Partial success possible: {recovery_plan['partial_success_possible']}"
        )
