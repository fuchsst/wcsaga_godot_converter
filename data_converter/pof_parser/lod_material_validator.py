...
class LODMaterialValidator:
    ...
    def validate_lod_hierarchy(
        self, hierarchy: LODHierarchy, model_data: Dict[str, Any]
    ) -> ValidationReport:
        ...
        try:
            ...
            self.logger.info(
                f"Validating LOD hierarchy for {hierarchy.base_model_name}"
            )
            ...
            if "materials" in model_data:
                ...
                report.material_validation = self._analyze_material_performance(
                    model_data["materials"]
                )

            # Validate collision data if provided
            if "collision_data" in model_data:
                collision_issues = self._validate_collision_data(
                    model_data["collision_data"]
                )
                report.validation_issues.extend(collision_issues)
                report.collision_validation = self._analyze_collision_performance(
                    model_data["collision_data"]
                )

            # Validate optimization results if provided
            if "optimization_results" in model_data:
                optimization_issues = self._validate_optimization_results(
                    model_data["optimization_results"]
                )
                report.validation_issues.extend(optimization_issues)
                report.optimization_validation = (
            ...
            report.overall_result = self._determine_overall_result(
    ...
    def _validate_materials(
        ...
        for i, material in enumerate(materials):
            ...
            if material.get("normal_enabled", False) and material.get(
    ...
    def _validate_collision_data(
        self, collision_data: Dict[str, Any]
    ) -> List[ValidationIssue]:
    ...
    def _analyze_lod_performance(self, hierarchy: LODHierarchy) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "distance_range": {
                ...
                "ratio": hierarchy.lod_levels[-1].distance_threshold
                / hierarchy.lod_levels[0].distance_threshold,
            },
        ...
        for i in range(1, len(hierarchy.lod_levels)):
            ...
            smoothness = (
                1.0
                - min(vertex_diff, 0.5)
                - min(distance_diff / prev_lod.distance_threshold, 1.0) * 0.2
            )
            smoothness_scores.append(max(0.0, smoothness))

        if smoothness_scores:
            analysis["transition_smoothness"] = sum(smoothness_scores) / len(
                smoothness_scores
            )

        return analysis

    def _analyze_material_performance(
        self, materials: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        analysis["shader_complexity"] = sum(complexity_scores) / max(
            len(complexity_scores), 1
        )

        return analysis

    def _analyze_collision_performance(
        self, collision_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if collision_type in ["sphere", "box"]:
            ...
            analysis["memory_estimate_bytes"] = (
                vertex_count * 12 + len(collision_data.get("indices", [])) * 4
            )
    ...
    def _analyze_optimization_effectiveness(
        ...
        analysis = {
            "vertex_reduction": optimization_results.get("vertex_reduction_ratio", 0.0),
            "triangle_reduction": optimization_results.get(
                "triangle_reduction_ratio", 0.0
            ),
            "memory_savings_mb": optimization_results.get("memory_savings_bytes", 0)
            / (1024 * 1024),
            "techniques_used": optimization_results.get(
        ...
        memory_score = min(
            analysis["memory_savings_mb"] * 10, 20
        )  # Up to 20 points for memory
    ...
    def _calculate_performance_metrics(
        ...
        if "optimization_results" in model_data:
            reduction = model_data["optimization_results"].get(
        ...
        if "material_validation" in model_data:
            ...
            metrics.shader_complexity_score = material_analysis.get(
                "shader_complexity", 1.0
            )
            metrics.transparency_overdraw = (
                material_analysis.get("transparency_ratio", 0.0) * 100
            )
    ...
    def _generate_recommendations(self, report: ValidationReport) -> List[str]:
        ...
        if report.performance_metrics.lod_transition_smoothness < 0.8:
            recommendations.append(
                "Adjust LOD distance thresholds for smoother transitions"
            )
        ...
        if report.performance_metrics.memory_usage_mb > 50:
            recommendations.append(
                "Optimize textures and vertex data to reduce memory usage"
            )

        if report.performance_metrics.shader_complexity_score > 3:
            recommendations.append(
                "Simplify shaders for better performance on mobile devices"
            )
    ...
    def _generate_optimization_suggestions(self, report: ValidationReport) -> List[str]:
        ...
        if report.performance_metrics.triangle_count > 20000:
            suggestions.append(
                "Consider more aggressive triangle reduction for distant LODs"
            )
    ...
    def save_validation_report(
        ...
        try:
            ...
            report_dict = {
                ...
                "validation_issues": [
                    issue.to_dict() for issue in report.validation_issues
                ],
                "detailed_results": {
