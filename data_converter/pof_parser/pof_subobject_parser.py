...
from .pof_types import SubObject, BoundingBox
...
def read_sobj_chunk(f: BinaryIO, length: int) -> Optional[SubObject]:
    ...
    try:
        ...
        if (
            ...
            or bounding_box.min.y > bounding_box.max.y
            or bounding_box.min.z > bounding_box.max.z
        ):
        ...
        if bytes_read < length:
            try:
                ...
                logger.error(
                    f"Failed to recover position after SOBJ parsing error: {seek_error}"
                )
        return None
