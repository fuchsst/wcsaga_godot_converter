...
class POFOBJConverter:
    ...
    def convert_pof_to_obj(
        self, pof_path: Path, obj_path: Path, texture_dir: Optional[Path] = None
    ) -> bool:
        """
        ...
        try:
            ...
            conversion_result = self._convert_data_to_obj(
                model_data, godot_data, texture_dir
            )
            ...
            if not self._write_mtl_file(
                conversion_result.materials, mtl_path, texture_dir
            ):
            ...
            logger.info(
                f"Conversion complete - Vertices: {stats['vertex_count']}, "
    ...
    def _convert_data_to_obj(
        self, model_data: Any, godot_data: Dict[str, Any], texture_dir: Optional[Path]
    ) -> Optional[OBJConversionResult]:
        ...
        try:
            ...
            for subobj in model_data.subobjects:
                ...
                for polygon in bsp_geometry["polygons"]:
                    material_name = self._get_material_name(
                        polygon["texture_index"], model_data.textures
                    )
                    ...
                    for triangle_indices in triangles:
                        ...
                        for vertex_idx in triangle_indices:
                            if vertex_idx >= len(bsp_geometry["vertices"]):
                                logger.warning(
                                    f"Invalid vertex index {vertex_idx} in {subobj_name}"
                                )
                                continue
                            ...
                            normal = (
                                bsp_geometry["normals"][vertex_idx]
                                if vertex_idx < len(bsp_geometry["normals"])
                                else (0.0, 0.0, 1.0)
                            )
                            uv = (
                                bsp_geometry["uvs"][vertex_idx]
                                if vertex_idx < len(bsp_geometry["uvs"])
                                else (0.0, 0.0)
                            )
            ...
            logger.error(
                f"Failed to convert POF data to OBJ format: {e}", exc_info=True
            )
            return None

    def _extract_bsp_geometry(
        self, filename: str, subobj: Dict[str, Any]
    ...
    def _create_materials(
        self, textures: List[str], texture_dir: Optional[Path]
        ...
        for idx, texture_name in enumerate(textures):
            if not texture_name or texture_name.lower() in ("none", ""):
                ...
                material_map[material_name] = OBJMaterial(
                    name=material_name, diffuse_color=(0.7, 0.7, 0.7)
                )
                continue
        ...
        if "default" not in material_map:
            material_map["default"] = OBJMaterial(
                name="default", diffuse_color=(0.5, 0.5, 0.5)
            )

        return material_map

    def _find_texture_file(
        self, texture_name: str, texture_dir: Optional[Path]
    ) -> Optional[str]:
    ...
    def _convert_position(
        ...
        return (
            ...
            -z
            * self.coordinate_scale,  # Z inverted (POF: +Z forward, Godot: -Z forward)
        )
        ...
        try:
            with open(obj_path, "w", encoding="utf-8") as f:
                ...
                for vertex in result.vertices:
                    f.write(
                        f"v {vertex.position[0]:.6f} {vertex.position[1]:.6f} {vertex.position[2]:.6f}\n"
                    )

                f.write("\n# Normals\n")
                for vertex in result.vertices:
                    f.write(
                        f"vn {vertex.normal[0]:.6f} {vertex.normal[1]:.6f} {vertex.normal[2]:.6f}\n"
                    )
    ...
    def _write_mtl_file(
        self, materials: List[OBJMaterial], mtl_path: Path, texture_dir: Optional[Path]
    ) -> bool:
        ...
        try:
            with open(mtl_path, "w", encoding="utf-8") as f:
                ...
                for material in materials:
                    ...
                    f.write(
                        f"Ka {material.ambient_color[0]:.3f} {material.ambient_color[1]:.3f} {material.ambient_color[2]:.3f}\n"
                    )
                    f.write(
                        f"Kd {material.diffuse_color[0]:.3f} {material.diffuse_color[1]:.3f} {material.diffuse_color[2]:.3f}\n"
                    )
                    f.write(
                        f"Ks {material.specular_color[0]:.3f} {material.specular_color[1]:.3f} {material.specular_color[2]:.3f}\n"
                    )
