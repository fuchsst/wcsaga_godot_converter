...
class POFValidator:
    ...
    def validate_model_data(
        ...
        error_handler: Optional[UnifiedPOFErrorHandler] = None,
    ) -> ValidationResult:
        """
    ...
    def _validate_version(
        self, version: POFVersion, errors: List[str], warnings: List[str]
    ) -> None:
        """Validate POF version compatibility."""
        if version < POFVersion.MIN_COMPATIBLE:
            errors.append(
                f"Version {version} is below minimum compatible version {POFVersion.MIN_COMPATIBLE}"
            )
            self.error_handler.add_error(
    ...
    def _validate_header(
        self, header: Any, errors: List[str], warnings: List[str]
    ) -> None:
        ...
        if header.mass < 0:
            ...
            self.error_handler.add_data_integrity_warning(
                f"Negative mass: {header.mass}", recovery_action="Use absolute value"
            )

        # Validate bounding box
        if (
            header.bounding_box.min.x > header.bounding_box.max.x
            or header.bounding_box.min.y > header.bounding_box.max.y
            or header.bounding_box.min.z > header.bounding_box.max.z
        ):
    ...
    def _validate_subobjects(
        self, subobjects: List[SubObject], errors: List[str], warnings: List[str]
    ) -> None:
        ...
        for i, subobj in enumerate(subobjects):
            ...
            if subobj.radius < 0:
                warnings.append(
                    f"Subobject {subobj.number} has negative radius: {subobj.radius}"
                )
                self.error_handler.add_data_integrity_warning(
            ...
            if subobj.has_bsp_data() and subobj.bsp_data_size <= 0:
                warnings.append(
                    f"Subobject {subobj.number} has invalid BSP data size: {subobj.bsp_data_size}"
                )
                self.error_handler.add_data_integrity_warning(
    ...
    def _validate_texture_references(
        self, model_data: POFModelData, errors: List[str], warnings: List[str]
    ) -> None:
        ...
        for tex_idx in texture_refs:
            if tex_idx > max_texture_index:
                errors.append(
                    f"Invalid texture index referenced: {tex_idx} (max: {max_texture_index})"
                )
                self.error_handler.add_validation_error(
    ...
    def _validate_bsp_trees(
        self, subobjects: List[SubObject], errors: List[str], warnings: List[str]
    ) -> None:
        """Validate BSP tree structures."""
        for subobj in subobjects:
            if subobj.bsp_tree:
                self._validate_bsp_node(
                    subobj.bsp_tree, f"subobject {subobj.number}", errors, warnings
                )

    def _validate_bsp_node(
        self, node: BSPNode, context: str, errors: List[str], warnings: List[str]
    ) -> None:
        ...
        if node.node_type == BSPNodeType.LEAF:
            ...
            for i, polygon in enumerate(polygons_to_validate):
                if len(polygon.vertices) < 3:
                    errors.append(
                        f"Invalid polygon in {context}: only {len(polygon.vertices)} vertices"
                    )
                    self.error_handler.add_validation_error(
    ...
    def _validate_gameplay_elements(
        self, model_data: POFModelData, errors: List[str], warnings: List[str]
    ) -> None:
        ...
        for i, wp in enumerate(model_data.gun_points + model_data.missile_points):
            if not (
                -1.0 <= wp.normal.x <= 1.0
                and -1.0 <= wp.normal.y <= 1.0
                and -1.0 <= wp.normal.z <= 1.0
            ):
                warnings.append(f"Weapon point {i} has non-normalized normal")
                self.error_handler.add_data_integrity_warning(
                    "Non-normalized weapon point normal",
        ...
        for i, dock in enumerate(model_data.docking_points):
            if not dock.name.strip():
                ...
                self.error_handler.add_data_integrity_warning(
                    "Unnamed docking point", recovery_action="Generate default name"
                )
    ...
    def _validate_spatial_consistency(
        ...
        for subobj in model_data.subobjects:
            ...
            if (
                sub_min.x < main_min.x
                or sub_min.y < main_min.y
                or sub_min.z < main_min.z
                or sub_max.x > main_max.x
                or sub_max.y > main_max.y
                or sub_max.z > main_max.z
            ):
                warnings.append(
                    f"Subobject {subobj.number} extends outside main bounds"
                )
                self.error_handler.add_data_integrity_warning(
...
def validate_pof_model(
    model_data: POFModelData, error_handler: Optional[UnifiedPOFErrorHandler] = None
) -> ValidationResult:
    """
