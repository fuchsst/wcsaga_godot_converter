...
import pytest
from .pof_error_handler import (
    POFErrorHandler, POFError, ErrorSeverity, ErrorCategory,
    handle_parsing_error, handle_validation_error, handle_compatibility_warning,
    clear_errors, get_error_summary
)
...
class TestPOFErrorHandler:
    ...
    def test_error_filtering(self):
        ...
        self.handler.add_error("Error 2", ErrorSeverity.WARNING, ErrorCategory.COMPATIBILITY)
    ...
    def test_error_summary(self):
        ...
        self.handler.add_error("Critical error", ErrorSeverity.CRITICAL, ErrorCategory.PARSING)
        self.handler.add_error("Normal error", ErrorSeverity.ERROR, ErrorCategory.VALIDATION)
        self.handler.add_error("Warning", ErrorSeverity.WARNING, ErrorCategory.COMPATIBILITY)
        self.handler.add_error("Another warning", ErrorSeverity.WARNING, ErrorCategory.DATA_INTEGRITY)
        ...
        assert summary['has_critical_errors'] == True
    ...
    def test_recovery_plan(self):
        ...
        assert recovery_plan['can_continue'] == True  # No critical errors
        assert recovery_plan['data_loss_expected'] == True
        assert recovery_plan['partial_success_possible'] == True
        assert len(recovery_plan['recommended_actions']) >= 2
        
        # Test with critical error
        self.handler.add_error("Critical failure", ErrorSeverity.CRITICAL, ErrorCategory.PARSING)
        recovery_plan_critical = self.handler.create_recovery_plan()
        assert recovery_plan_critical['can_continue'] == False
    ...
    def test_convenience_methods(self):
        ...
        compatibility_errors = self.handler.get_errors(category=ErrorCategory.COMPATIBILITY)
        integrity_errors = self.handler.get_errors(category=ErrorCategory.DATA_INTEGRITY)
...
class TestErrorRecoveryScenarios:
    ...
    def test_chunk_parsing_recovery(self):
        ...
        self.handler.add_parsing_error("Missing required field", recovery_action="use default value")
        
        self.handler.set_position(0x300)
        self.handler.add_data_integrity_warning("Data checksum mismatch", recovery_action="verify manually")
        
        # Check recovery plan
        recovery_plan = self.handler.create_recovery_plan()
        
        assert recovery_plan['can_continue'] == True
        assert recovery_plan['data_loss_expected'] == True
        assert any("skip" in action.lower() for action in recovery_plan['recommended_actions'])
    ...
    def test_version_compatibility_recovery(self):
        ...
        self.handler.add_compatibility_warning(
            "Chunk format outdated",
            recovery_action="apply compatibility patch"
        )
        
        recovery_plan = self.handler.create_recovery_plan()
        
        assert recovery_plan['can_continue'] == True
        assert recovery_plan['data_loss_expected'] == False  # Warnings only
        assert any("fallback" in action.lower() for action in recovery_plan['recommended_actions'])
...
def test_comprehensive_error_scenario():
    ...
    handler.add_parsing_error("Invalid radius value", recovery_action="use calculated radius")
    
    # SOBJ chunk parsing
    handler.set_chunk_context(0x4F424A32, "SOBJ")
    handler.set_position(0x200)
    handler.add_validation_error("Invalid parent index", recovery_action="attach to root")
    
    # TXTR chunk parsing
    handler.set_chunk_context(0x54585452, "TXTR")
    handler.set_position(0x300)
    handler.add_data_integrity_warning("Texture name contains invalid characters", 
                                     recovery_action="sanitize name")
