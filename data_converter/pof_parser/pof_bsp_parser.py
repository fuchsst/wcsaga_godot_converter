...
class BSPParser:
    ...
    def parse_bsp_tree(self, bsp_data: bytes, version: int) -> Optional[BSPNode]:
        ...
        try:
            ...
            chunk_type, chunk_data, remaining_data = self._parse_chunk_header(
                bsp_data, False
            )
            ...
            logger.info(
                f"BSP tree parsed successfully: {self.stats.node_count} nodes, {self.stats.polygon_count} polygons"
            )
            return bsp_tree
    ...
    def _parse_bsp_node(self, buf: bytes, version: int) -> Optional[BSPNode]:
        ...
        try:
            ...
            if chunk_type in (BSPChunkType.SORTNORM, BSPChunkType.SORTNORM2):
                return self._parse_sortnorm_node(
                    chunk_data, next_chunk, version, chunk_type
                )
                ...
                logger.warning(
                    f"Unhandled BSP chunk type: {chunk_type.name if chunk_type in BSPChunkType else f'0x{chunk_type:08X}'}"
                )
                return BSPNode(
                    node_type=BSPNodeType.EMPTY,
                    normal=Vector3D(0, 0, 1),
    ...
    def _parse_sortnorm_node(
        self,
        chunk_data: bytes,
        next_chunk: bytes,
        version: int,
        chunk_type: BSPChunkType,
    ) -> BSPNode:
        ...
        try:
            ...
            if len(next_chunk) > 20:  # Rough estimate for minimum chunk size
                back_child = (
                    self._parse_bsp_node(next_chunk[20:], version) or back_child
                )
    ...
    def _parse_boundbox_node(
        self, chunk_data: bytes, next_chunk: bytes, version: int
    ) -> BSPNode:
        ...
        try:
            ...
            while current_buf and len(current_buf) >= 8:
                try:
                    poly_chunk_type, poly_chunk_data, poly_next_chunk = (
                        self._parse_chunk_header(current_buf, False)
                    )

                    if poly_chunk_type == BSPChunkType.ENDOFBRANCH:
                        break
                    elif poly_chunk_type in (
                        BSPChunkType.TMAPPOLY,
                        BSPChunkType.FLATPOLY,
                    ):
                        polygon = self._parse_polygon(
                            poly_chunk_data, poly_chunk_type, version
                        )
                        if polygon:
    ...
    def _parse_polygon(
        self, chunk_data: bytes, chunk_type: BSPChunkType, version: int
    ) -> Optional[BSPPolygon]:
        ...
        try:
            ...
            if chunk_type in (
                BSPChunkType.TMAPPOLY,
                BSPChunkType.FLATPOLY,
                BSPChunkType.TMAPPOLY2,
            ):
                if (
                    chunk_type != BSPChunkType.TMAPPOLY2
                ):  # TMAPPOLY2 doesn't have center/radius
                    reader.read_vector3d()
                    reader.read_float32()
    ...
    def _parse_chunk_header(
        self, buf: bytes, is_subchunk: bool
    ) -> Tuple[BSPChunkType, bytes, bytes]:
        ...
        if chunk_size > 1000000:  # Reasonable limit for chunk size
            logger.warning(
                f"Suspiciously large chunk size {chunk_size}, treating as invalid"
            )
            chunk_size = 0

        if chunk_size > len(buf) - 8:
            raise ValueError(
                f"Chunk size {chunk_size} exceeds buffer length {len(buf) - 8}"
            )
    ...
    def _calculate_bbox_from_children(
        self, front: Optional[BSPNode], back: Optional[BSPNode]
        ...
        if (
            ...
            or bbox.max.x == float("-inf")
            or bbox.max.y == float("-inf")
            or bbox.max.z == float("-inf")
        ):
            return None
