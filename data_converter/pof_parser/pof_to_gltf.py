...
try:
    ...
    Node = Mesh = Primitive = Attributes = Accessor = BufferView = Buffer = Material = (
        None
    )
...
TYPE_MAP = {
    ...
    4: "VEC4",
    9: "MAT3",
    16: "MAT4",  # Although MAT3 is 9 floats, MAT4 is 16
}
    ...
    if len(arr.shape) == 1:  # Scalar
        ...
        if gltf_type is None:
            ...
            if num_components == 9 and arr.shape[0] * arr.shape[1] == 9:  # MAT3 check
                ...
                num_components == 16 and arr.shape[0] * arr.shape[1] == 16
            ):  # MAT4 check
...
def convert_pof_to_gltf(
    ...
    def convert_pos(v: List[float]) -> List[float]:
        return [
            v[0] * coordinate_scale,
            v[1] * coordinate_scale,
            -v[2] * coordinate_scale,
        ]
    ...
    for subobj_index, subobj in enumerate(pof_data.subobjects):
        ...
        subobj_node_map[node._pof_number] = (
            len(gltf.nodes) - 1
        )  # Map POF number to GLTF node index
    ...
    for subobj_index, subobj in enumerate(pof_data.subobjects):
            ...
            try:
                with open(pof_file_path, "rb") as f:
                    ...
                    if len(bsp_data_bytes) != bsp_data_size:
                        logger.error(
                            f"Failed to read expected {bsp_data_size} bytes of BSP data for subobject {subobj_num}. Got {len(bsp_data_bytes)}."
                        )
                        bsp_data_bytes = None  # Mark as failed
            except Exception as e:
                logger.error(
                    f"Error reading BSP data for subobject {subobj_num} from {pof_file_path}: {e}"
                )
                bsp_data_bytes = None
        else:
            logger.debug(
                f"Subobject {subobj_num} has no BSP data (offset={bsp_data_offset}, size={bsp_data_size})."
            )

        if not bsp_data_bytes:
            logger.warning(
                f"No BSP data read for subobject {subobj_num}. Skipping geometry."
            )
            continue
        ...
        if not parsed_bsp or not parsed_bsp.get("vertices"):
            logger.warning(
                f"Failed to parse BSP data or no vertices found for subobject {subobj_num}. Skipping geometry."
            )
            continue
        ...
        for poly in parsed_bsp["polygons"]:
            ...
            for idx in poly["indices"]:
                if idx < 0 or idx >= num_subobj_verts:
                    logger.error(
                        f"Invalid local vertex index {idx} in polygon for subobject {subobj_num}. Max verts: {num_subobj_verts}. Skipping polygon."
                    )
                    valid_poly = False
                    break
        ...
        if node_index is not None:
            ...
            gltf.nodes[node_index]._material_indices = list(
                set(p["texture_index"] for p in parsed_bsp["polygons"])
            )
    ...
    vertices_np = (
        np.array(all_vertices_np, dtype=np.float32)
        if all_vertices_np
        else np.array([], dtype=np.float32)
    )
    normals_np = (
        np.array(all_normals_np, dtype=np.float32)
        if all_normals_np
        else np.array([], dtype=np.float32)
    )
    uvs_np = (
        np.array(all_uvs_np, dtype=np.float32)
        if all_uvs_np
        else np.array([], dtype=np.float32)
    )
    ...
    # Vertex BufferView
    buffer_views_info.append(
        (0, len(buffer_bytes_list[0]), 34962)
    )  # 34962 = ARRAY_BUFFER
    ...
    indices_accessors = {}
    indices_buffer_views_start_offset = sum(
        bv[1] for bv in buffer_views_info if bv is not None
    )
    ...
    for material_index, indices in sorted(all_indices_by_material.items()):
        ...
        padding = (
            alignment - (len(b"".join(buffer_bytes_list)) % alignment)
        ) % alignment
        ...
        buffer_views_info.append(
            (current_indices_offset, bv_len, 34963)
    ...
    for bv_info in buffer_views_info:
        if bv_info is not None:  # Skip placeholders
            ...
            if target == 34962:  # ARRAY_BUFFER
                if current_bv_index == 0:
                    vertex_bv_idx = len(gltf.bufferViews) - 1
                elif current_bv_index == 1:
                    normal_bv_idx = len(gltf.bufferViews) - 1
                elif current_bv_index == 2:
                    uv_bv_idx = len(gltf.bufferViews) - 1
            current_bv_index += 1
        else:
    ...
    for idx, tex_name in enumerate(pof_data.textures):
        ...
        image_index = next(
            (i for i, img in enumerate(gltf.images) if img.uri == image_uri), -1
        )
        if image_index == -1:
    ...
    for node_idx, node in enumerate(gltf.nodes):
        ...
        if hasattr(node, "_material_indices"):
            processed_materials = (
                set()
            )  # Track materials already added to this node's mesh
            for pof_mat_idx in node._material_indices:
                if pof_mat_idx in primitives_by_material:
                    ...
                    if -1 in primitives_by_material and -1 not in processed_materials:
                        ...
                        pof_mat_idx != -1
                    ):  # Don't warn for expected -1 indices if default exists
                        logger.warning(
                            f"Node {node_idx} ('{node.name}') uses POF material index {pof_mat_idx}, but no corresponding primitive found."
                        )
        ...
        if node_primitives:
            ...
            logger.warning(
                f"Node {node_idx} ('{node.name}') had material indices but no primitives were generated."
            )
    ...
    for i, node in enumerate(gltf.nodes):
        if i == 0:
            continue  # Skip root node itself
        ...
        if parent_pof_num != -1:
            ...
            if parent_node_index is not None and parent_node_index < len(gltf.nodes):
                ...
                logger.warning(
                    f"Parent node for POF subobject {parent_pof_num} not found for child node {i} ('{node.name}'). Attaching to root."
                )
        ...
        if hasattr(node, "_pof_parent"):
            del node._pof_parent
        if hasattr(node, "_pof_number"):
            del node._pof_number
        if hasattr(node, "_vertex_start"):
            del node._vertex_start
        if hasattr(node, "_vertex_count"):
            del node._vertex_count
        if hasattr(node, "_material_indices"):
            del node._material_indices
    ...
    if gltf.scenes[0].nodes != [0]:
        logger.warning(
            f"Scene nodes were not [0], correcting. Original: {gltf.scenes[0].nodes}"
        )
    ...
    for i, node in enumerate(gltf.nodes):
        ...
        if parent_pof_num != -1:
            ...
            if parent_node_index is not None and parent_node_index < len(gltf.nodes):
                ...
                logger.warning(
                    f"Parent node for POF subobject {parent_pof_num} not found. Attaching node {i} to root."
                )
                root_children.append(i)
        else:
            # No parent specified, attach to root (node 0)
...
if __name__ == "__main__":
    ...
    dummy_pof_data = {
        ...
        "header": {
            ...
            "obj_flags": 0,
            "num_subobjects": 1,
            "min_bounding": [-5.0, -5.0, -10.0],
            "max_bounding": [5.0, 5.0, 10.0],
        ...
        "objects": [
            {
                ...
                "bounding_min": [-5.0, -5.0, -10.0],
                "bounding_max": [5.0, 5.0, 10.0],
                "name": "subobject0",
                "properties": "",
                "movement_type": -1,
                "movement_axis": -1,
                "bsp_data_size": 50,
                "bsp_data_offset": 1000,  # Placeholder offset
            }
        ],
        "special_points": [],
        "paths": [],
