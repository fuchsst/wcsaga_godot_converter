...
class BSPChunkType(IntEnum):
    """BSP chunk types for BSP tree parsing."""
...
class Vector3D:
    ...
    def __init__(self, x: float = 0.0, y: float = 0.0, z: float = 0.0):
        """Initialize Vector3D with validation."""
        ...
        if any(
            not isinstance(v, (int, float)) or not abs(v) < float("inf")
            for v in [x, y, z]
        ):
    ...
    def magnitude(self) -> float:
        """Alias for length() for consistency with vector3d.py."""
        return self.length()
...
class BSPNode:
    ...
    def __post_init__(self):
        ...
        if self.node_type == BSPNodeType.NODE:
            ...
            if abs(normal_length - 1.0) > 1e-3:
                raise ValueError(
                    f"Splitting normal must be unit length, got {normal_length}"
                )
...
class POFModelData:
    ...
    def __post_init__(self):
        ...
        if self.version < POFVersion.MIN_COMPATIBLE:
            raise ValueError(
                f"Version {self.version} is below minimum compatible version {POFVersion.MIN_COMPATIBLE}"
            )

        # Check subobject consistency
        if len(self.subobjects) != self.header.num_subobjects:
            raise ValueError(
                f"Header reports {self.header.num_subobjects} subobjects but found {len(self.subobjects)}"
            )
        ...
        for subobj in self.subobjects:
            if subobj.bsp_tree:
                texture_indices.update(
                    self._get_texture_indices_from_bsp(subobj.bsp_tree)
                )

        for tex_idx in texture_indices:
            if tex_idx >= len(self.textures):
                raise ValueError(
                    f"Subobject references invalid texture index {tex_idx}"
                )
...
def dict_to_header(data: Dict[str, Any], version: POFVersion) -> POFHeader:
    ...
    return POFHeader(
        ...
        moment_of_inertia=[
            Vector3D(*row)
            for row in data.get("moment_inertia", [[1, 0, 0], [0, 1, 0], [0, 0, 1]])
        ],
