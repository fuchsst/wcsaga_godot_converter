...
class HelpTableConverter(BaseTableConverter):
    ...
    def _init_parse_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "text": re.compile(r'^\+text\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+XSTR\("(.+)",\s*-1\)', re.IGNORECASE),
            "line": re.compile(r'^\+pline\s+(\d+)\s+(.+)', re.IGNORECASE),
            "right_bracket": re.compile(r'^\+right_bracket\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)', re.IGNORECASE),
            "left_bracket": re.compile(r'^\+left_bracket\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)', re.IGNORECASE)
        }
    ...
    def convert_to_godot_resource(
        ...
        return {
            ...
            "overlays": {
                entry["name"]: self._convert_help_overlay_entry(entry) for entry in entries
            },
