...
class FireballTableConverter(BaseTableConverter):
    ...
    def _init_parse_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "shockwave_speed": re.compile(
                r"^\+Shockwave Speed:\s*([\d\.]+)$", re.IGNORECASE
            ),
    ...
    def parse_entry(self, state: ParseState) -> Optional[Dict[str, Any]]:
        ...
        while state.has_more_lines():
            ...
            if "name" in fireball_data:
                ...
                if self._parse_patterns["fireball_start"].match(
                    line
    ...
    def _parse_fireball_property(
        self, line: str, fireball_data: Dict[str, Any]
    ) -> bool:
        ...
        for field, (min_val, max_val) in numeric_fields.items():
            if field in entry:
                ...
                if not isinstance(value, (int, float)) or not (
                    ...
                    self.logger.warning(
                        f"Fireball {entry['name']}: Invalid {field} value: {value}"
                    )
                    return False
