...
class StarsTableConverter(BaseTableConverter):
    ...
    def parse_table(self, state: ParseState) -> List[Dict[str, Any]]:
        ...
        while state.has_more_lines():
            ...
            if match:
                entries["bitmaps"].append(
                    {"name": match.group(1).strip(), "type": "background"}
                )
                continue
            ...
            if match:
                entries["debris"].append(
                    {"name": match.group(1).strip(), "type": "debris"}
                )
                continue
            ...
            if match:
                entries["debris_neb"].append(
                    {"name": match.group(1).strip(), "type": "debris_nebula"}
                )
                continue
