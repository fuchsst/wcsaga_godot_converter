...
class SpeciesDefsTableConverter(BaseTableConverter):
    ...
    def _init_parse_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "fred_color": re.compile(
                r"^\$FRED Color:\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)$",
                re.IGNORECASE,
            ),
            "debris_texture": re.compile(r"^\+Debris_Texture:\s*(.+)$", re.IGNORECASE),
            "shield_hit_ani": re.compile(r"^\+Shield_Hit_ani:\s*(.+)$", re.IGNORECASE),
            "thrust_anim": re.compile(
                r"^\+(Pri|Sec|Ter)_(Normal|Afterburn):\s*(.+)$", re.IGNORECASE
            ),
    ...
    def parse_entry(self, state: ParseState) -> Optional[Dict[str, Any]]:
        ...
        while state.has_more_lines():
            ...
            if match:
                ...
                entry_data["thrust_anims"][
                    f"{anim_type.lower()}_{anim_state.lower()}"
                ] = anim_name.strip()
                continue
    ...
    def convert_to_godot_resource(
        ...
        return {
            ...
            "species": {
                entry["name"]: self._convert_species_entry(entry) for entry in entries
            },
