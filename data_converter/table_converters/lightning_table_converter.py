...
class LightningTableConverter(BaseTableConverter):
    ...
    def _init_parse_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "storm_flavor": re.compile(
                r"^\+flavor:\s*([\d\.]+)\s+([\d\.]+)\s+([\d\.]+)$", re.IGNORECASE
            ),
            "storm_random_freq": re.compile(
                r"^\+random_freq:\s*(\d+)\s+(\d+)$", re.IGNORECASE
            ),
            "storm_random_count": re.compile(
                r"^\+random_count:\s*(\d+)\s+(\d+)$", re.IGNORECASE
            ),
