...
from .interfaces import IFileConverter, IDirectoryConverter, IBatchConverter, IValidatableConverter, IConfigurableConverter, IProgressReporter
...
class BaseDataConverter(ABC, IFileConverter, IDirectoryConverter):
    ...
    def validate_conversion(self, source_path: Path, target_path: Path) -> bool:
        ...
        if not target_path.exists():
            self.logger.error(f"Validation failed: target file does not exist: {target_path}")
            return False
        
        if target_path.stat().st_size == 0:
            self.logger.warning(f"Validation warning: target file is empty: {target_path}")
            return False
