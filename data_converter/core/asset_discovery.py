...
from .catalog.asset_catalog import AssetRelationship
...
class AssetDiscoveryEngine:
    ...
    def __init__(self, source_dir: Path):
        ...
        self.faction_patterns = {
            ...
            "pirate": {"fighters": ["prf_"], "ships": ["prs_"], "general": ["pr_"]},
            "border_worlds": {
                "fighters": ["bwf_"],
                "ships": ["bws_"],
                "general": ["bw_"],
        ...
        self.audio_categories = {
            ...
            "shield_sounds": ["shield_", "hull_", "armor_", "damage_", "impact_on_"],
            "ui_sounds": [
                "button_",
                "menu_",
                "alert_",
            ...
            "ambient_sounds": ["ambient_", "background_", "env_", "atmosphere_"],
            "explosion_sounds": [
                "explosion_",
                "blast_",
                "boom_",
    ...
    def discover_entity_assets(
        ...
        if entity_type == "ship":
            relationships.extend(
                self._discover_ship_assets(entity_name, primary_model_path)
            )
        elif entity_type == "weapon":
            relationships.extend(
                self._discover_weapon_assets(entity_name, primary_model_path)
            )
    ...
    def _discover_ship_assets(
        self, ship_name: str, model_path: Optional[str] = None
    ) -> List[AssetRelationship]:
        ...
        if model_path:
            ...
            ship_texture_patterns.extend(
                [f"{model_stem}*", f"{model_stem}_*", f"{model_stem}-*"]
            )
            ...
            for pattern in ship_texture_patterns:
                for texture_file in textures_dir.glob(pattern):
                    if texture_file.suffix.lower() in self.asset_extensions["texture"]:
                        rel_type = self._determine_texture_relationship_type(
                            texture_file.stem
                        )
                        relationships.append(
                            AssetRelationship(
                                source_path=str(
                                    texture_file.relative_to(self.source_dir)
                                ),
    ...
    def _discover_weapon_assets(
        self, weapon_name: str, model_path: Optional[str] = None
    ) -> List[AssetRelationship]:
        ...
        if animations_dir.exists():
            for pattern in weapon_effect_patterns:
                for effect_file in animations_dir.glob(f"{pattern}.eff"):
                    relationships.extend(
                        self._create_effect_relationships(effect_file, weapon_name)
                    )
    ...
    def _discover_effect_assets(self, effect_name: str) -> List[AssetRelationship]:
        ...
        if animations_dir.exists():
            ...
            for eff_file in animations_dir.glob(f"{eff_pattern}.eff"):
                relationships.extend(
                    self._create_effect_relationships(eff_file, effect_name)
                )

        return relationships

    def _discover_textures(
        self, entity_name: str, model_path: Optional[str] = None
    ) -> List[AssetRelationship]:
    ...
    def _discover_animations(self, entity_name: str) -> List[AssetRelationship]:
        ...
        for pattern in animation_patterns:
            for eff_file in animations_dir.glob(f"{pattern}.eff"):
                relationships.extend(
                    self._create_effect_relationships(eff_file, entity_name)
                )
    ...
    def _determine_texture_relationship_type(self, texture_stem: str) -> str:
        ...
        if any(suffix in stem_lower for suffix in ["_normal", "-normal"]):
            ...
            suffix in stem_lower for suffix in ["_spec", "-spec", "_shine", "-shine"]
        ):
            return "specular"
        elif any(
            suffix in stem_lower for suffix in ["_glow", "-glow", "_emit", "-emit"]
        ):
            return "emission"
    ...
    def _create_effect_relationships(
        self, eff_file: Path, entity_name: str
    ) -> List[AssetRelationship]:
            ...
            for category, prefixes in categories.items():
                for prefix in prefixes:
                    if entity_lower.startswith(prefix):
                        logger.debug(
                            f"Entity '{entity_name}' classified as {faction} ({category})"
                        )
                        return faction
    ...
    def classify_entity_subcategory(self, entity_name: str, faction: str) -> str:
        ...
        if any(pattern in entity_lower for pattern in ["fighter", "f-", "interceptor"]):
            ...
            pattern in entity_lower for pattern in ["missile", "rocket", "torpedo"]
        ):
            return "missiles"
        elif any(
            pattern in entity_lower
            for pattern in ["destroyer", "cruiser", "carrier", "corvette"]
        ):
            ...
            for suffix in suffixes:
                for ext in self.asset_extensions["texture"]:
                    ...
                    if material_file.exists():
                        found_materials[material_type] = str(
                            material_file.relative_to(self.source_dir)
                        )
                        found = True
                        break
                if found:
    ...
    def discover_effect_sequences(self, effect_name: str) -> List[AssetRelationship]:
        ...
        sequence_pattern = re.compile(
            rf"{re.escape(effect_name)}_(\d{{4}})\.dds$", re.IGNORECASE
        )
        sequence_frames = []
        ...
        if sequence_frames:
            logger.debug(
                f"Found {len(sequence_frames)} effect frames for '{effect_name}'"
            )
    ...
    def analyze_faction_distribution(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_factions": len(
                [f for f, stats in faction_stats.items() if sum(stats.values()) > 0]
            ),
        }
