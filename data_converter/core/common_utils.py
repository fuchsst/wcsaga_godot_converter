...
class ConversionUtils:
    ...
    def load_table_file(table_file: Path) -> Optional[List[str]]:
        ...
        try:
            ...
            logging.getLogger(__name__).error(f"Failed to load table file {table_file}: {e}")
            return None
    ...
    def create_resource_file(content: str, target_path: Path) -> bool:
        ...
        try:
            ...
            logging.getLogger(__name__).error(f"Failed to create resource file {target_path}: {e}")
            return False
...
class ValidationFramework:
    ...
    def validate_conversion_output(target_path: Path, min_size: int = 1) -> bool:
        ...
        if not target_path.exists():
            logging.getLogger(__name__).error(f"Output file does not exist: {target_path}")
            return False
        
        if target_path.stat().st_size < min_size:
            logging.getLogger(__name__).warning(f"Output file is empty or too small: {target_path}")
            return False
            
        return True
    
    @staticmethod
    def validate_resource_content(content: str, required_fields: List[str] = None) -> bool:
        """Validate Godot resource file content."""
        if not content.startswith('[gd_resource'):
            logging.getLogger(__name__).error("Invalid resource format: missing [gd_resource] header")
            return False
        
        if required_fields:
            for field in required_fields:
                if field not in content:
                    logging.getLogger(__name__).warning(f"Resource missing required field: {field}")
                    return False
...
class TableTypeDetector:
    ...
    TABLE_PATTERNS = {
        ...
        'iff_defs': {
            'filename': ['iff'],
            'content': ['#iffs', '$iff name:']
        },
        'ai_profiles': {
            'filename': ['ai_profiles'],
            'content': ['#ai profiles']
        },
        'asteroid': {
            'filename': ['asteroid'],
            'content': ['#asteroid types']
        },
        'cutscenes': {
            'filename': ['cutscenes'],
            'content': ['#cutscenes']
        ...
        'scripting': {
            'filename': ['scripting'],
            'content': ['#scripting hooks']
        },
        'sounds': {
            'filename': ['sounds'],
            'content': ['#sound entries']
        },
        'stars': {
            'filename': ['stars'],
            'content': ['#star types']
