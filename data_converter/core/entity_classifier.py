...
class EntityClassifier:
    ...
    def classify_entity(
        ...
        return self._classify_by_naming_patterns(
            entity_name, entity_lower, source_context
        )
    ...
    def _validate_ship_classification(
        self, entity_name: str, entity_lower: str
    ) -> EntityType:
        """
        ...
        for weapon_name in self.known_weapons:
            if weapon_name in entity_lower:
                logger.debug(
                    f"Entity '{entity_name}' reclassified as weapon (found in ships.tbl)"
                )
        ...
        if any(indicator in entity_lower for indicator in weapon_indicators):
            logger.debug(
                f"Entity '{entity_name}' reclassified as weapon (weapon indicator)"
            )
            return EntityType.WEAPON
    ...
    def _classify_by_naming_patterns(
        self, entity_name: str, entity_lower: str, source_context: Optional[str] = None
    ) -> EntityType:
    ...
    def detect_faction(self, entity_name: str) -> str:
        ...
        sorted_prefixes = sorted(
            self.faction_prefixes.items(), key=lambda x: len(x[0]), reverse=True
        )

        for prefix, faction in sorted_prefixes:
            if entity_lower.startswith(prefix):
                logger.debug(
                    f"Entity '{entity_name}' classified as {faction} faction (prefix: {prefix})"
                )
                return faction
    ...
    def classify_entity_subcategory(self, entity_name: str, faction: str) -> str:
        ...
        if any(pattern in entity_lower for pattern in ["fighter", "f-", "interceptor"]):
            ...
            for pattern in ["destroyer", "cruiser", "carrier", "corvette", "frigate"]
        ):
            return "capital_ships"
        elif any(
            pattern in entity_lower
            for pattern in ["installation", "station", "base", "platform"]
        ):
    ...
    def classify_by_file_extension(self, file_path: Path) -> EntityType:
        ...
        extension_mapping = {
            ...
            ".dds": (
                ...
                if "interface" in parent_dir
                else EntityType.EFFECT
            ),
            ".pcx": (
                EntityType.UI_ELEMENT
                if "interface" in parent_dir
    ...
    def _classify_pof_file(self, pof_path: Path) -> EntityType:
        ...
        if any(indicator in stem for indicator in ["debris", "rock"]):
            ...
            indicator in stem for indicator in ["shockwave", "warp", "subspace", "jump"]
        ):
            return EntityType.EFFECT
