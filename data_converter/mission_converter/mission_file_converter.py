...
class MissionFileConverter:
    ...
    def convert_mission_file(
        self, fs2_path: Path, output_dir: Path, validate_output: bool = True
    ) -> ConversionResult:
        ...
        try:
            ...
            result.statistics = self._calculate_conversion_statistics(
                mission_data, converted_events
            )
    ...
    def _parse_mission_file(
        self, fs2_path: Path, result: ConversionResult
    ) -> Optional[MissionData]:
    ...
    def _convert_mission_events(
        self, mission_data: MissionData, result: ConversionResult
    ) -> Dict[str, ConvertedEvent]:
    ...
    def _generate_godot_scene(
        self, mission_data: MissionData, output_dir: Path, result: ConversionResult
    ) -> List[str]:
        """Generate Godot scene files."""
        try:
            # Create output paths
            mission_name = self._sanitize_filename(
    ...
    def _generate_mission_resources(
        self,
    ...
    def _generate_resource_scripts(
        self, output_dir: Path, result: ConversionResult
    ) -> List[str]:
        """Generate resource script files."""
        try:
            script_files = self.resource_generator.generate_mission_script_resources(
                output_dir
            )
    ...
    def _generate_sexp_documentation(
        self,
        mission_data: MissionData,
        converted_events: Dict[str, ConvertedEvent],
        output_dir: Path,
        ...
        try:
            ...
            docs_path = (
                output_dir / "docs" / "missions" / f"{mission_name}_sexp_mapping.md"
            )

            # Generate documentation content
            docs_content = self._create_sexp_documentation_content(
                mission_data, converted_events
            )
    ...
    def _create_mission_resource_content(
        self, mission_data: MissionData, converted_events: Dict[str, ConvertedEvent]
    ) -> str:
...
"""
    ...
    def _create_mission_metadata(
        ...
        metadata = {
            ...
            "wing_summary": [
                {"name": wing.name, "ships": wing.ships, "num_waves": wing.num_waves}
            ...
            "goal_summary": [
                {
                    ...
                    "converted": goal.name in converted_events
                    or f"goal_{goal.name}" in converted_events,
                }
            ...
            "conversion_info": {
                ...
                "total_functions_generated": len(converted_events)
                * 2,  # Each event gets check + action function
                "sexp_formulas_processed": len(
                    [e for e in mission_data.events if e.formula]
                )
    ...
    def _create_sexp_documentation_content(
        self, mission_data: MissionData, converted_events: Dict[str, ConvertedEvent]
    ) -> str:
...
"""
            ...
            try:
                ...
                if not content.startswith("[gd_resource"):
                    result.errors.append(
                        f"VALIDATION: Invalid resource format: {resource_file}"
                    )
                    continue
                ...
                if "script =" not in content:
                    result.warnings.append(
                        f"VALIDATION: Resource missing script reference: {resource_file}"
                    )
                ...
                if "/mission_data.gd" in content:
                    ...
                    self._validate_ship_instance_resource(
                        content, resource_file, result
                    )
                ...
                result.errors.append(
                    f"VALIDATION: Failed to validate resource {resource_file}: {e}"
                )

    def _validate_mission_data_resource(
        self, content: str, file_path: str, result: ConversionResult
    ) -> None:
        ...
        for field in required_fields:
            if field not in content:
                result.errors.append(
                    f"VALIDATION: Mission resource missing {field}: {file_path}"
                )

    def _validate_ship_instance_resource(
        self, content: str, file_path: str, result: ConversionResult
    ) -> None:
        ...
        for field in required_fields:
            if field not in content:
                result.errors.append(
                    f"VALIDATION: Ship resource missing {field}: {file_path}"
                )

    def _validate_event_resource(
        self, content: str, file_path: str, result: ConversionResult
    ) -> None:
        """Validate event resource content."""
        required_fields = ["event_name =", "trigger_conditions =", "actions ="]

        for field in required_fields:
            if field not in content:
                result.errors.append(
                    f"VALIDATION: Event resource missing {field}: {file_path}"
                )
        ...
        for scene_file in scene_files:
            try:
                ...
                if not content.startswith("[gd_scene"):
                    result.errors.append(
                        f"VALIDATION: Invalid scene format: {scene_file}"
                    )
                    continue
                ...
                if 'type="MissionController"' not in content:
                    result.errors.append(
                        f"VALIDATION: Scene missing MissionController: {scene_file}"
                    )
                ...
                for container in required_containers:
                    if f'name="{container}"' not in content:
                        result.warnings.append(
                            f"VALIDATION: Scene missing {container} container: {scene_file}"
                        )

            except Exception as e:
                result.errors.append(
                    f"VALIDATION: Failed to validate scene {scene_file}: {e}"
                )
    ...
    def _validate_data_consistency(self, result: ConversionResult) -> None:
        ...
        try:
            ...
            if not mission_name:
                result.warnings.append(
                    "VALIDATION: Mission name not set for consistency check"
                )
                return
            ...
            main_resource_files = [
                ...
                if f.endswith(f"{self._sanitize_filename(mission_name)}.tres")
            ]
    ...
    def _validate_resource_references(
        self, main_resource_path: str, result: ConversionResult
    ) -> None:
            ...
            if ship_match:
                ...
                for ship_ref in ship_refs:
                    if ship_ref.startswith("res://"):
                        ...
                        file_path = ship_ref.replace(
                            "res://",
                            str(Path(main_resource_path).parent.parent.parent) + "/",
                        )
                        if not Path(file_path).exists():
                            result.warnings.append(
                                f"VALIDATION: Referenced ship resource not found: {ship_ref}"
                            )
            ...
            if wing_match:
                ...
                for wing_ref in wing_refs:
                    if wing_ref.startswith("res://"):
                        file_path = wing_ref.replace(
                            "res://",
                            str(Path(main_resource_path).parent.parent.parent) + "/",
                        )
                        if not Path(file_path).exists():
                            result.warnings.append(
                                f"VALIDATION: Referenced wing resource not found: {wing_ref}"
                            )
            ...
            if event_match:
                ...
                for event_ref in event_refs:
                    if event_ref.startswith("res://"):
                        file_path = event_ref.replace(
                            "res://",
                            str(Path(main_resource_path).parent.parent.parent) + "/",
                        )
                        if not Path(file_path).exists():
                            result.warnings.append(
                                f"VALIDATION: Referenced event resource not found: {event_ref}"
                            )

        except Exception as e:
            result.errors.append(
                f"VALIDATION: Resource reference validation failed: {e}"
            )

    def _calculate_conversion_statistics(
        self, mission_data: MissionData, converted_events: Dict[str, ConvertedEvent]
    ) -> Dict[str, Any]:
        """Calculate conversion statistics."""
        total_sexp_formulas = len([e for e in mission_data.events if e.formula]) + len(
            [g for g in mission_data.goals if g.formula]
        )

        ship_classes = set(
            obj.class_name for obj in mission_data.objects if obj.class_name
        )
        ...
        return {
            ...
            "conversion_success": {
                "total_events_goals": len(mission_data.events)
                + len(mission_data.goals),
                "converted_events": len(converted_events),
                "conversion_rate": len(converted_events)
                / max(len(mission_data.events) + len(mission_data.goals), 1),
                "sexp_formulas_processed": total_sexp_formulas,
                "functions_generated": len(converted_events) * 2,
            },
            "data_preservation": {
                "ship_data_preserved": all(
                    obj.name and obj.class_name for obj in mission_data.objects
                ),
                "wing_data_preserved": all(wing.name for wing in mission_data.wings),
                "objective_data_preserved": all(
                    goal.name or goal.message for goal in mission_data.goals
                ),
            },
        }
...
if __name__ == "__main__":
    ...
    def main():
        parser = argparse.ArgumentParser(
            description="Convert FS2 mission files to Godot format"
        )
        parser.add_argument(
            "input", type=Path, help="Input FS2 mission file or directory"
        )
        parser.add_argument(
            "--output",
            "-o",
        ...
        parser.add_argument(
            "--asset-path", default="res://", help="Base asset path for Godot resources"
        )
        parser.add_argument(
            "--validate", action="store_true", help="Validate conversion output"
        )
        parser.add_argument(
        ...
        if args.input.is_file():
            ...
            result = converter.convert_mission_file(
