...
class MissionResourceGenerator:
    ...
    def generate_mission_resources(
        self,
        ...
        try:
            ...
            mission_resource_path = (
                output_dir / "resources" / "missions" / f"{mission_name}.tres"
            )
            if self._generate_mission_resource(
                mission_data, converted_events, mission_resource_path
            ):
            ...
            ship_resources = self._generate_ship_resources(
                mission_data.objects, ships_dir
            )
            ...
            wing_resources = self._generate_wing_resources(
                mission_data.wings, wings_dir
            )
            ...
            event_resources = self._generate_event_resources(
                converted_events, events_dir
            )
            resource_files.extend(event_resources)

            # 5. Waypoint Resources
            waypoints_dir = (
                output_dir / "resources" / "missions" / mission_name / "waypoints"
            )
            waypoint_resources = self._generate_waypoint_resources(
                mission_data.waypoints, waypoints_dir
            )
    ...
    def _generate_mission_resource(
        self,
        mission_data: MissionData,
        converted_events: Dict[str, ConvertedEvent],
        output_path: Path,
...
"""
    ...
    def _generate_ship_resources(
        self, objects: List[MissionObject], output_dir: Path
    ) -> List[str]:
...
"""
                ...
                self.logger.error(
                    f"Failed to generate ship resource for {obj.name}: {e}"
                )

        return resource_files

    def _generate_wing_resources(
        self, wings: List[MissionWing], output_dir: Path
    ) -> List[str]:
...
"""
                ...
                self.logger.error(
                    f"Failed to generate wing resource for {wing.name}: {e}"
                )

        return resource_files

    def _generate_event_resources(
        self, converted_events: Dict[str, ConvertedEvent], output_dir: Path
    ) -> List[str]:
...
"""
                ...
                self.logger.error(
                    f"Failed to generate event resource for {event_name}: {e}"
                )

        return resource_files

    def _generate_waypoint_resources(
        self, waypoints: List[MissionWaypoint], output_dir: Path
    ) -> List[str]:
            ...
            try:
                ...
                for wp in waypoint_list:
                    waypoint_positions.append(
                        f"Vector3({wp.position[0]}, {wp.position[1]}, {wp.position[2]})"
                    )
...
"""
                ...
                self.logger.error(
                    f"Failed to generate waypoint resource for {list_name}: {e}"
                )
    ...
    def generate_mission_script_resources(self, output_dir: Path) -> List[str]:
        ...
        self.logger.info(
            f"Using existing mission resource scripts: {len(script_files)} files found"
        )
        return script_files
    ...
    def _get_departure_location_enum(self, location: str) -> int:
        ...
        location_mapping = {"hyperspace": 0, "docking_bay": 1}
        return location_mapping.get(location.lower(), 0)
