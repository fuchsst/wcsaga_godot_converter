...
"""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        state = ParseState(lines=ai_profiles_content.split('\n'), filename="test_ai_profiles.tbl")
        entries = converter.parse_table(state)
        
        assert len(entries) == 2, f"Expected 2 AI profile entries, got {len(entries)}"
        assert entries[0]['name'] == 'SAGA RETAIL', "First profile should be SAGA RETAIL"
        assert entries[1]['name'] == 'TEST PROFILE', "Second profile should be TEST PROFILE"
        assert 'difficulty_scales' in entries[0], "Profile should have difficulty scales"
