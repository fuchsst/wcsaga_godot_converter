import pytest
import os
from data_converter.table_converters.scripting_table_converter import ScriptingTableConverter
...
class TestScriptingTableConverter:
    ...
    def scripting_content(self):
        script_path = os.path.join(
            ...
            '..', '..', '..', 'source_assets', 'wcs_hermes_campaign', 'hermes_core', 'scripting.tbl'
        )
    ...
    def test_parse_scripting_table(self, converter, scripting_content):
        ...
        assert len(entries) >= 2, f"Expected at least 2 script hooks, got {len(entries)}"
    ...
    def test_validate_entries(self, converter, scripting_content):
        ...
        for entry in entries:
            assert converter.validate_entry(entry), f"Entry validation failed for hook {entry['hook']}"