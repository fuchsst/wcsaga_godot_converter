...
"""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        state = ParseState(lines=credits_content.split('\n'), filename="test_credits.tbl")
        ...
        assert len(entries[0]['credit_lines']) == 6, f"Expected 6 credit lines, got {len(entries[0]['credit_lines'])}"
        assert entries[0]['credit_lines'][0] == 'Game Developer', "First credit line should be 'Game Developer'"
        assert entries[0]['credit_lines'][1] == 'John Doe', "Second credit line should be 'John Doe'"
...
"""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert len(entries) == 0, f"Expected 0 entries for empty content, got {len(entries)}"
...
"""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert len(entries[0]['credit_lines']) == 2, f"Expected 2 credit lines, got {len(entries[0]['credit_lines'])}"