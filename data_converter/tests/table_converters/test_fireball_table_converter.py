...
from data_converter.table_converters.fireball_table_converter import (
    FireballTableConverter,
)
...
"""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        state = ParseState(
            lines=fireball_content.split("\n"), filename="test_fireball.tbl"
        )
        entries = converter.parse_table(state)

        assert len(entries) == 2, f"Expected 2 fireball entries, got {len(entries)}"
        assert (
            entries[0]["name"] == "Explosion01"
        ), "First fireball should be Explosion01"
        assert (
            entries[1]["name"] == "Explosion02"
        ), "Second fireball should be Explosion02"
