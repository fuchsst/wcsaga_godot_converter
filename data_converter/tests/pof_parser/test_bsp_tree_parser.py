...
from data_converter.pof_parser.pof_bsp_parser import (
    BSPParser,
    parse_bsp_data,
    BSPChunkType,
)
...
class TestBSPTreeParser(unittest.TestCase):
    ...
    def test_deffpoints_parsing(self):
        ...
        bsp_data.extend(
            struct.pack("<I", 68)
        )  # chunk size (content only: 4 + 4 + 3*12 + 2*12 = 8 + 36 + 24 = 68)
        ...
        self.assertEqual(
            bsp_tree.node_type,
            BSPNodeType.EMPTY,
            "Should return EMPTY node for DEFFPOINTS-only data",
        )
    ...
    def test_tmappoly_parsing(self):
        ...
        bsp_data.extend(
            struct.pack("<I", 63)
        )  # chunk size (content only: 4 + 4 + 3*12 + 1*12 = 8 + 36 + 12 = 56)
        bsp_data.extend(struct.pack("<I", 3))  # num vertices
    ...
    def test_boundbox_with_polygons(self):
        ...
        self.assertEqual(
            bsp_tree.node_type, BSPNodeType.LEAF, "Should create LEAF node"
        )
