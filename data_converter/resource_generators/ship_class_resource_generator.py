...
class ShipClassResourceGenerator(ResourceGenerator):
    ...
    def generate_resource(self, asset_id: str) -> Optional[str]:
        ...
        try:
            ...
            resource_content = self._create_ship_class_resource_content(
                asset_id, ship_data
            )
        ...
        try:
            ...
            for section in required_sections:
                if section not in content:
                    logger.error(
                        f"Missing required section {section} in {resource_path}"
                    )
                    return False
    ...
    def _create_ship_class_resource_content(
        self, asset_id: str, ship_data: ShipClassData
    ) -> str:
        """
    ...
    def _extract_ship_properties(self, ship_data: ShipClassData) -> Dict[str, Any]:
        ...
        properties = {
            ...
            "use_newtonian_dampening": getattr(
                ship_data, "use_newtonian_dampening", False
            ),
            ...
            "afterburner_fuel_capacity": getattr(
                ship_data, "afterburner_fuel_capacity", 0.0
            ),
            ...
            "afterburner_forward_accel": getattr(
                ship_data, "afterburner_forward_accel", 0.0
            ),
            ...
            "explosion_shockwave_speed": getattr(
                ship_data, "explosion_shockwave_speed", 0.0
            ),
            # Impact effects
            "impact_spew_max_particles": getattr(
                ship_data, "impact_spew_max_particles", 0
            ),
            # Damage effects
            "damage_spew_max_particles": getattr(
                ship_data, "damage_spew_max_particles", 0
            ),
            # Debris properties
            ...
            "debris_min_rotation_speed": getattr(
                ship_data, "debris_min_rotation_speed", 0.0
            ),
            "debris_max_rotation_speed": getattr(
                ship_data, "debris_max_rotation_speed", 0.0
            ),
            ...
            "debris_damage_multiplier": getattr(
                ship_data, "debris_damage_multiplier", 0.0
            ),
            "debris_lightning_arc_percent": getattr(
                ship_data, "debris_lightning_arc_percent", 0.0
            ),
        }
        ...
        if (
            hasattr(ship_data, "texture_replacements")
            and ship_data.texture_replacements
        ):
    ...
    def generate_ship_subsystems(self, asset_id: str) -> List[str]:
        ...
        try:
            ...
            if not ship_data_dict:
                logger.warning(
                    f"No ship data found for subsystem generation: {asset_id}"
                )
                return subsystem_paths
            ...
            for i, subsystem in enumerate(ship_data.subsystems):
                try:
                    ...
                    subsystem_content = self._create_subsystem_resource_content(
                        subsystem
                    )
                    ...
                    if self._write_resource_file(subsystem_content, subsystem_path):
                        ...
                        logger.error(
                            f"Failed to write subsystem resource: {subsystem_path}"
                        )
    ...
    def _generate_default_hardpoint_configuration(
        self, ship_name: str
    ) -> Dict[str, Any]:
        """
        ...
        # Capital ships
            ...
            for pattern in ["capital", "cruiser", "destroyer", "carrier"]
        ):
            hardpoints = {
