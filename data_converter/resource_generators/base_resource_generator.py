...
class ResourceGenerator(ABC):
    ...
    def validate_resources(self, resource_paths: List[Union[str, Path]]) -> Dict[str, bool]:
        """
        ...
        for resource_path in resource_paths:
            try:
                ...
                logger.debug(f"Resource validation {'passed' if is_valid else 'failed'}: {resource_path}")
    ...
    def _create_tres_header(
        ...
        """
        header_lines = [
            f"[gd_resource type=\"{resource_type}\""
        ]
    ...
    def _get_asset_faction(self, asset_name: str) -> str:
        ...
        terran_patterns = [
            "gtf_", "gtb_", "gtc_", "gtd_", "gtt_", "gta_",
            "confed", "terran", "tc_", "hermes"
        ]
        ...
        # Kilrathi patterns
        kilrathi_patterns = [
            "kaf_", "kab_", "kac_", "kad_", "kat_", "k_", 
            "kilrathi", "dralthi", "salthi", "gratha", "jalthi"
        ]
        ...
        pirate_patterns = [
            "pirate", "bandit", "corsair", "privateer"
        ]
    ...
    def _get_asset_category(self, asset_name: str, asset_type: str) -> str:
        ...
        if asset_type == "ship":
            if any(pattern in name_lower for pattern in ["fighter", "interceptor", "stealth"]):
                ...
                return "cruisers"
            elif any(pattern in name_lower for pattern in ["capital", "dreadnought", "carrier"]):
                return "capital_ships"
            elif any(pattern in name_lower for pattern in ["transport", "cargo", "freighter"]):
                return "transports"
            else:
        ...
        # Weapon categories
            ...
            if any(pattern in name_lower for pattern in ["laser", "beam"]):
                return "beams"
            elif any(pattern in name_lower for pattern in ["missile", "torpedo", "rocket"]):
                return "missiles"
